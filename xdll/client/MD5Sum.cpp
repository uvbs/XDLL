...
int MD5Sum::Calculate2(string strContent)
{
	return Calculate2((const unsigned char*)strContent.c_str(),strContent.size());
}

int MD5Sum::Calculate2(const unsigned char* pachSource, unsigned int nLen)
{
	...
	{
		int nPart[4];
		nPart[0] = rawHash[2*i]<<24;
		nPart[1] = rawHash[2*i+1]<<16;
		nPart[2] = rawHash[2*i+2]<<8;
		nPart[3] = rawHash[2*i+3];
		int part4 = nPart[0] | nPart[1] | nPart[2] | nPart[3];
		nSum ^= part4;
	}
	return nSum;
}