...
BOOL NetUtils::DownloadFile(tstring strServer, tstring strObject, tstring strProxy, tstring strFileFullPath, HWND hWnd)
{
	...
	tstring::size_type  index_port=strServer.find(':');
    if (index_port==tstring::npos)
    {
	...
	{
		tstring  str_port=strServer.substr(index_port+1);
		strServer=strServer.substr(0,strServer.size()-str_port.size()-1);
		dwPort=atoi(str_port.c_str());
	}
	...
	try
	{
		if (strProxy.size() == 0)
		{
			...
			if (!(hOpen = InternetOpen(NULL, LOCAL_INTERNET_ACCESS, NULL, 0, 0)))
			{
				throw(StrUtils::Format(_T("E 1==%u"), GetLastError()));
			}
		...
		else
		{
			...
			if (!(hOpen = InternetOpen(NULL,  CERN_PROXY_INTERNET_ACCESS, strProxy.c_str(), NULL, 0)))
			{

				throw(StrUtils::Format(_T("E 2==%u"), GetLastError()));
			}
		...
		{
			throw(StrUtils::Format(_T("E 3==%u"), GetLastError()));
		}
		...
		{
			throw(StrUtils::Format(_T("E 4==%u"), GetLastError()));
		}
		...
		{
			throw(StrUtils::Format(_T("E 5==%u"), GetLastError()));
		}
		...
		{
			throw(StrUtils::Format(_T("E 6==%u"), GetLastError()));
		}


		if (dwCode < 200 || dwCode > 299)
		{
			throw(StrUtils::Format(_T("dwCode==%u"), dwCode));
		}
		...
		{
			throw(StrUtils::Format(_T("E 7==%u"), GetLastError()));
		} 
		...
		{
			throw(StrUtils::Format(_T("E 8==%u"), strFileFullPath, GetLastError()));
		}
		...
		do
		{
			...
			{
				throw(StrUtils::Format(_T("E 9==%u"), GetLastError()));
			}
			...
			if (dwSize != 0)
			{
				...
				{
					throw(StrUtils::Format(_T("E 10==%u"), GetLastError()));
				}
		...
		{
			throw(StrUtils::Format(_T("E 11==%u"), GetLastError()));
		}

		if (!InternetCloseHandle(hConnect))
		{
			throw(StrUtils::Format(_T("E 12==%u"), GetLastError()));
		}

		if (!InternetCloseHandle(hOpen))
		{
			throw(StrUtils::Format(_T("E 13==%u"), GetLastError()));
		}
